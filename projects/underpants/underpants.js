...
_.reject = function(arr, func){
  return  _.filter(arr, function(e,i,a){
      return !func(e, i, a);
      });
}
...
_.pluck = function(arrObj, prop){
   return _.map(arrObj, function(ele, i, arr){
        return arr[i][prop]; 
...
_.every = function(coll, test){
 let counter = 0;
  _.each(coll, function(val, call, file){
    if(!test){
        if(!val){
            counter++;
        }
...
//     if(seed === undefined){
//     seed = arr[0];
//       _.each(arr, function (ele, ind, arr){
//      seed = func(seed, ele, ind);
//     })
//     }
//     else {
//       _.each(arr, function (ele, ind, arr){
